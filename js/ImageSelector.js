    ...
    async beforeRegisterNodeDef(nodeType, nodeData, app) {
        if (nodeData.name === "CRZImageSelector") {
			...
			// Helpers to compute rects on-demand (avoid relying on draw-time caches)
				...
				// Leave padding for resize handles and output sockets
				const padding = 4;
				const rightPadding = 20; // Extra space for output sockets
				const availableWidth = this.size[0] - padding - rightPadding;
				const availableHeight = this.size[1] - (padding * 2);

				// Calculate cell size to fill the available space
				const cellWidth = availableWidth / 3;  // 3 columns
				const cellHeight = availableHeight / 2; // 2 rows

				// Each cell fills exactly 1/6th of the available space
			...
			nodeType.prototype.onMouseDown = function(event, pos, graphcanvas) {
				...
				if (this.isResizing) {
					return false; // Let LiteGraph handle resize
				}
			...
			nodeType.prototype._getClearButtonRect = function(index) {
				...
				const clearButtonSize = Math.min(12, Math.min(rect.width, rect.height) * 0.2); // Scale with smaller dimension
				const x = rect.x + rect.width - clearButtonSize - 2;
				const y = rect.y + 2;
            ...
            nodeType.prototype.onDrawForeground = function(ctx) {
                ...
                widgetNames.forEach((widgetName, index) => {
                    ...
                    if (this.thumbnails[widgetName]) {
                        ...
                        ctx.beginPath();
                        ctx.roundRect(x, y, thumbSize, rect.height, 8);
                        ctx.fill();
                        
                        // Calculate aspect ratio to fit entire image in cell while maintaining aspect ratio
                        const aspectRatio = img.width / img.height;
                        const cellAspectRatio = rect.width / rect.height;
                        let drawWidth, drawHeight, offsetX = 0, offsetY = 0;
                        
                        if (aspectRatio > cellAspectRatio) {
                            // Image is wider than cell - fit to cell width, scale height down
                            drawWidth = rect.width;
                            drawHeight = rect.width / aspectRatio;
                            offsetY = (rect.height - drawHeight) / 2;
                        } else {
                            // Image is taller than cell - fit to cell height, scale width down
                        ...
                        // Draw full-resolution image scaled down to fit entirely in cell
                        ctx.drawImage(img, x + offsetX, y + offsetY, drawWidth, drawHeight);
                        ctx.restore();
                        
                        // Draw clear button as overlay on top-right corner of image
                        const clearButtonSize = Math.min(12, Math.min(rect.width, rect.height) * 0.2); // Scale with smaller dimension
                        const clearButtonX = x + rect.width - clearButtonSize - 2;
                        ...
                        ctx.font = `12px Arial`;
                        ctx.textAlign = "center";
                        ctx.fillText("Upload", x + rect.width/2, y + rect.height/2 + 4);
